...
class Detr3DHead(DETRHead):
    ...
    def loss(self,
        ...
        # print(gt_maps[0].shape, map_logits[0].shape)
        # [1, 1, 256, 256] [1, 2, 256, 256]

        for i in range(len(gt_maps)):
            pred_map = map_logits[i]
            gt_map = gt_maps[i].squeeze(0)
            gt_map = gt_map
            loss_map = self.loss_map(pred_map, gt_map)
            if i == 0:
                loss_dict['loss_fvmap'] = loss_map
            else:
                loss_dict['loss_bevmap'] = loss_map
        return loss_dict
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for i in range(num_samples):
            ...
            bboxes = CameraInstance3DBoxes(bboxes, 7)
            scores = preds['scores']
            labels = preds['labels']
