...
def convert_kitti_to_json(root_dir, split_text, output_path):
    annos = []
    images = []
    label_dir = os.path.join(root_dir, 'training', 'label_2')
    global_id = 0
    split_path = os.path.join(root_dir, 'ImageSets', split_text)

    # 1. 从 split_path 加载需要处理的 image_ids
    with open(split_path, 'r') as f:
        split_ids = set(line.strip() for line in f if line.strip())
    print(f'Loaded {len(split_ids)} ids from {split_path}')
    ...
    # print(f"Saved {len(annos)} annotations to {output_path}")

# 你还需确保你有 get_default_camera_info() 和 kitti_category_id() 这两个函数在环境中


convert_kitti_to_json(root_dir='data/dair_kitti', split_text='train.txt', output_path='data/dair_kitti/train.json')
convert_kitti_to_json(root_dir='data/dair_kitti', split_text='val.txt', output_path='data/dair_kitti/val.json')